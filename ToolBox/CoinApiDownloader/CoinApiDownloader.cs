...
{
    /// <summary>
    /// CoinApi Data Downloader class
    /// </summary>
    public class CoinApiDownloader : IDataDownloader
    {
        private readonly string _exchange;

        /// <summary>
        /// Initializes a new instance of the <see cref="CoinApiDownloader"/> class
        /// </summary>
        ...
        public CoinApiDownloader(string exchange = Market.GDAX)
        {
            _exchange = exchange;
        }
        ...
        public IEnumerable<BaseData> Get(Symbol symbol, Resolution resolution, DateTime startUtc, DateTime endUtc)
        {
            ...
            while (counter <= endUtc)
            {
                while (hour < 24)
                {
                    ...
                    {
                        var request =
                            $"http://CoinApi.io/api/marketdata/ticker/{_exchange}/{symbol.Value}/{counter.ToStringInvariant("yyyy-MM-dd")}/{hour}";
                        var data = cl.DownloadString(request);

                        yield return null;
                        /*
                        var mbtc = JsonConvert.DeserializeObject<List<object>>(data);
